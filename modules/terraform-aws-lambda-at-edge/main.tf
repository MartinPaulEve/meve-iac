...
data "archive_file" "zip_file_for_lambda" {
  ...
  source_file = "${var.lambda_code_source_dir}/${var.lambda_code_filename}"
...
resource "aws_s3_bucket_object" "artifact" {
  bucket = var.s3_artifact_bucket[0].id
  key    = "${var.name}.zip"
...
resource "aws_lambda_function" "lambda" {
  ...
  s3_bucket         = var.s3_artifact_bucket[0].id
  ...
  # always set this to us-east-1 for lambda edge
  provider = aws.lambda-edge
