...
# Cloudfront distribution for secondary s3 site.
resource "aws_cloudfront_distribution" "www_s3_distribution-secondary" {
  ...
  aliases = ["www.${var.secondary_domain_name}", var.secondary_domain_name]
  ...
  viewer_certificate {
    acm_certificate_arn      = aws_acm_certificate_validation.cert_validation-secondary.certificate_arn
    ssl_support_method       = "sni-only"
    minimum_protocol_version = "TLSv1.1_2016"
  }
